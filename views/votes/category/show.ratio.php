...
{% block title %}Visa kategori{% endblock %}
{% block pageId %}show-category{% endblock %}
{% block body %}
<section x-data="{ openVoteModal: null, openInfoModal: null }">
{% if($category) : %}
  <div class="flex items-start justify-between gap-4 mb-5">
    <h1 class="text-3xl font-semibold break-words">{{ $category->getAttribute('category') }}</h1>
  </div>
  <div class="w-full md:max-w-2/3">
    <button
      x-on:click="$refs.desc.classList.toggle('hidden')"
      class="font-semibold mb-3 bg-gray-50 text-gray-500 hover:bg-gray-100 transition-colors duration-300 cursor-pointer"
    >
      <span class="flex items-end gap-0.5">
        <span class="text-sm">Kategoribeskrivning</span>
    ...
    <div x-ref="desc" class="hidden">
      <p class="mb-3">{{ $category->getAttribute('description') }}</p>
    </div>
  </div>
{% if($subjects['data']) : %}
  <div class="flex items-start justify-between gap-4 mb-2">
    <h3 class="text-[20px] font-semibold">Ämnen</h3>
    <div class="text-xs text-gray-600 font-medium hidden md:block mt-2">
...
{% foreach($subjects['data'] as $subject) : %}
      <tr class="text-left border-b border-gray-200 hover:bg-gray-100 even:bg-white odd:bg-gray-50">
        ...
        <td data-cell="röster" class="md:align-top px-1.5 md:px-3 py-2.5 max-md:py-1.5 text-sm max-md:before:content-[attr(data-cell)] max-md:grid max-md:grid-cols-[1fr_2fr] max-md:gap-1 max-md:before:font-semibold max-md:before:text-sm max-md:before:capitalize">
    ...
    >
      ...
      >
        ...
        <form
          ...
          x-bind:action="'{{ route('votes.vote.index', ['id' => 0]) }}'.replace('0', openVoteModal)"
          method="post"
          class="mt-4"
          x-on:submit.prevent="
...
{% if($subjects['pagination']['total'] > $subjects['pagination']['per_page']) : %}
  ...
  <div class="mt-3">
    {{ paginate_links($subjects['pagination'], 'votes.category.show', 2, ['id' => $category->getAttribute('id')])|raw }}
  </div>
{% endif; %}
{% else : %}
...
{% endif; %}
{% else : %}
  <h1 class="text-3xl mb-8">Kategori</h1>
  <p>Kategorin kunde inte hittas.</p>
{% endif %}
</section>
{% endblock %}