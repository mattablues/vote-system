...
{% foreach($subjects['data'] as $subject) : %}
          <tr class="text-left border-b border-gray-200 hover:bg-gray-100 even:bg-white odd:bg-gray-50">
            <td data-cell="id" class="md:align-top px-1.5 md:px-3 py-2.5 max-md:py-1.5 text-sm max-md:before:content-[attr(data-cell)] max-md:grid max-md:grid-cols-[1fr_2fr] max-md:gap-1 max-md:before:font-semibold max-md:before:text-sm max-md:before:capitalize">{{ $subject->getAttribute('id') }}</td>
            <td data-cell="ämne" class="w-full md:align-top px-1.5 md:px-3 py-2.5 max-md:py-1.5 text-sm max-md:before:content-[attr(data-cell)] max-md:grid max-md:grid-cols-[1fr_2fr] max-md:gap-1 max-md:before:font-semibold max-md:before:text-sm max-md:before:capitalize">{{ $subject->getAttribute('subject') }}</td>
            <td data-cell="kategori" class="w-full md:align-top px-1.5 md:px-3 py-2.5 max-md:py-1.5 text-sm max-md:before:content-[attr(data-cell)] max-md:grid max-md:grid-cols-[1fr_2fr] max-md:gap-1 max-md:before:font-semibold max-md:before:text-sm max-md:before:capitalize">{{ $subject->getRelation('category')->getAttribute('category') }}</td>
            <td data-cell="åtgärd" class="ml-auto px-1.5 md:px-3 py-2.5 max-md:py-1.5 text-sm max-md:before:content-[attr(data-cell)] max-md:grid max-md:grid-cols-[1fr_2fr] max-md:gap-1 max-md:before:font-semibold max-md:before:text-sm max-md:before:capitalize">
              <div class="flex items-center gap-1.5 pb-1.5 md:pb-0">
                {% if(!$subject->getAttribute('published')) : %}
                <form action="{{ route('admin.subject.publish', ['id' => $subject->getAttribute('id')]) }}" method="post">
                  {{ csrf_field()|raw }}
                  <button class="inline-flex items-center text-xs font-semibold bg-green-500 text-white py-1 px-2 rounded hover:bg-green-600 active:bg-green-700 transition-colors cursor-pointer">Publicera</button>
                </form>
                {% else : %}
                <span class="inline-block text-xs font-semibold bg-gray-200/70 text-gray-400 py-1 px-2 rounded">Publicera</span>
                {% endif; %}
                <a href="{{ route('admin.subject.edit', ['id' => $subject->getAttribute('id')]) }}" class="inline-flex items-center text-xs font-semibold bg-blue-600 text-white py-1 px-2 rounded hover:bg-blue-700 active:bg-blue-800 transition-colors cursor-pointer">Redigera</a>
        ...
        >
          ...
          >
            ...
            <p class="mt-3 text-sm text-gray-700">
              Detta kommer att ta bort ämnet och associerade röster. Åtgärden kan inte ångras.
            </p>
            ...
            <div class="mt-5 flex justify-end gap-2">
              ...
              <form action="{{ route('admin.subject.delete', ['id' => $subject->getAttribute('id')]) }}" method="post">
                {{ csrf_field()|raw }}
...
{% if($subjects['pagination']['total'] > $subjects['pagination']['per_page']) : %}
      <div class="flex flex-wrap items-center justify-between gap-3 mt-1">
        ...
        <span class="block text-xs font-medium text-gray-600">sida {{ $subjects['pagination']['current_page'] }} av {{ calculate_total_pages($subjects['pagination']['total'], $subjects['pagination']['per_page']) }}</span>
      </div>
      <div class="mt-3">
        {{ paginate_links($subjects['pagination'], 'admin.subject.index', 2)|raw }}
      </div>
{% endif; %}
{% else : %}
