...
{% block pageId %}login-voter{% endblock %}
{% block body %}
    <section>
      <div class="layout-aside-right [--aside-right-w:250px]">
        <div class="area-content">
          <h1 class="text-3xl font-semibold mb-8">Logga in</h1>

          <form action="{{ route('voter.auth.create') }}" method="post" class="w-full bg-white max-w-xl">
            {{ csrf_field()|raw }}
            {{ honeypot_field()|raw }}
            ...
            <div class="relative mb-8">
              <div class="flex gap-x-2 items-center">
                <button class="whitespace-nowrap text-sm py-2 px-3 border border-blue-600 bg-blue-600 hover:bg-blue-700  transition-all duration-300 text-white rounded-lg cursor-pointer">Logga in</button>
                <a href="{{ route('voter.password-forgot.index') }}" class="text-sm text-blue-600 hover:text-blue-800 transition-all duration-300  text-left">Glömt lösenord?</a>
